...
We need permissions for Lambda and were looking at some ready policies first, but it seems that those give quite general permissions and we wanted to write better policies now when using CFN, so we decided not to use those but are writing our own role instead.
...
— we have to use containers to build as we have a different runtime on Gitpod. Added to sam build ``--use-container`` flag for this reason. 
