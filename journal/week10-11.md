...
The ddb-folder had to be organized differently to the previous stacks as there was difficulties for the template to access the Lambda coein another folder. The SAM file also had to be separated in build, package and deploy-files for it to work correctly. 

![ddb folder](assets/ddb.png)

Same kind of re-organizing could be done for other stacks as well, which could make the project easier to navigate. 
...
The solution for this is that we have to use containers to build as we have a different runtime on Gitpod. Added to sam build ``--use-container`` flag for to implement this. 

AFter this deployment worked and creaated a change set:
...
The diagram for the DynamoDB service is very simple as the structure is very straighforfard. All services are located outside of the VPC:

![DynamoDB diagram](assets/dynamo_diagram.png)

## CFN CI/CD

The implementation of CFN for CI/CD was done with similar structure as other CFN stacks. ``Template.yaml`` and ``config.toml`` -files were created in the CI/CD folder. 

The diffference in this stack is that it is going to include a nested stack. A nested stack can be used when you have code that you would like to re-use. Added this as a resource on template.yaml:
...
We haven’t yet incorporated CDK code into CloudFormation and we might not be able to as it takes time. But it would be a good practise to have everything in CloudFormation so that you can simply tear it down and re-deploy easily.
...
The frtonend was previously implemented as a Fargate container. Now a decision was made to change the architecture to CloudFront instead as this is something that had been discussed throughout the bootcampl
...
The CFN for frontend creates the following resources. As it creates also new A records an on Route 53 hosted zone, the previously created one had to be removed for  this to work.  It also creates two S3 buckets, one for the root domain and another for the WWW domain:
...
The complete diagram can be viewed in [Lucidchart](https://lucid.app/lucidchart/c0367745-b41d-4b29-8371-39111ea4907d/edit?viewport_loc=386%2C1137%2C2772%2C1383%2C0_0&invitationId=inv_aa6e85b3-2f6b-4908-879f-38eca7f7b88d). It contains all of the above parts that were created during this week's bootcamp and additionally the serverless image processing diagram that I have created during week 6-7.
