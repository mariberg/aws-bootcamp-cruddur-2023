...
As the frontend was originally built as a container, some changes were needed for the new implementation with CloudFront and S3. A new ``static build`` file was created that now builds the frontend with ``npm build`` instead of ``docker build``. 

We are using a library called ``aws_s3_website_sync``, which Andrew Brown has made especially for syncing a folder from your local dev environment to the S3 bucket and then invalidating the CloudFront cache. We created a ``sync`` Ruby script that creates a change set based on the built frontend. Executing the change set then uploads the files to the S3 bucket and creates a CloudFront invalidation:
...
The status in the beginning of the week was that our frontend had a CORS error and is not connected to data. The backend has a successful health check, but backend routes don’t return any data.
...
After the database was connected, we tried to register new users in the application. The Post Confirmation Lambda that is used to finish the creation of a new user had several issues which had to be fixed for this to work. The Lambda had the old database URL as an environment variable, so this had to be updated to the new one. The correct way to reference the connection string would be using parameter store, however, currently it is still listed directly as an environment variable. Then we noticed that the Lambda is inside the default VPC instead of the new VPC we have created. So that needed to be changed and a new security group needed to be created. The new Lambda security group was then added to the database's inbound security group rules. 

&nbsp;

## Fix CORS to use a domain name for the web app

The service ``template.yaml`` had '*' as allowed origin, which had to be corrected to make the application more secure. The correct frontend and backend urls were added to the ``config.toml`` and after that, the service was re-deployed.

&nbsp;

## Ensure CI/CD pipeline works and create activity works

In the beginning of this week creating a new Crud resulted in a 500 error. We modified several frontend files to use ``cognito_user_id`` instead of ``user handle``. I had also some hardcoded values there that had to be removed. Added actual users to ``seed.sql`` so that I can use in development the same users that I have in prod. I also found several small bugs in my application and after correcting all those, it was possible to create activities.
...
We wanted to remove all of the hardcoded values from different functions and this would have required a lot of manual changes in many places. Hence it would make more sense to use a decorator for this.

Added ``jwt_required function`` into ``cognito_jwt_token.py``. This function can now be imported into other files and used in other functions. This allows us to simplify the way the functions work. 
...
Created new files in lib-folder for Xray, Honeycomb, Rollbar, CORS and moved all code from app.py related to those to the corresponding files. Then imported and initialized them in app.py
...
Removed ``Replies`` from ``ActivityItem.js`` and created a new file ``Replies.js``.
...
We wanted our app to display some kind of error when a user tries to submit an empty form or a form with errors. Added ``FormErrors.js`` and ``FormErrorItem.js``

Started to go through all the different forms that we have and import in them the new FormErrors component. After implementing this, all of the forms will display better error messages to users so that they can understand what is going on.
...
After updating all  forms with error handling, we also had to update all functions where fetch is being used.  We looked at all the different fetch functions we have but ignored the ones that are not calling our backend (presigned URL for example).

Updated a lot of files in total. What is in common now is that everything is importing lib/Requests and for this reason, the post,put, get requests in individual components have been simplified a lot and the code is a lot shorter and easier to understand.

Added to ``Request.js`` ``options``. The idea is that we can have some routes that require authorization and others that don’t.
...
Created another new file ``ActivityShowPage.js``. We copied to it a lot of content from ``Activityfeed.js`` and we are not renaming everything so that we can simply use the same css.
...
Created a new file ``ActivityShowItem.js``. Changed ``ActivityShowpage.js`` to display ``ActivityShowItem`` instead of ``ActivityItem``.
...
To fix the application to use production DynamoDB, the following steps were completed:

- updated all tables names in ``ddb.py`` from hardcoded 'cruddur-messages' to 'os.getenv("DDB_MESSAGE_TABLE")'
...
Made some changes to css so that we can go to the home page and each item will have a different colour while we hover it and we can then click it and an individual Crud will open (it would display replies if those exist).  

One issue we had with the frontend application was that we had to manually refresh the page after writing a reply in order to see the new message. This was fixed by adding ``setReplies`` to ``activityShowPage``. 
...
Now it was time to move on to make sure that all changes we have made are implemented in production. We ran the database migration scripts now against production so that the ``reply_to_activity_uuid`` type was changed from integer to uuid.

Created a pull request to merge the main into prod to update the backend via CodePipeline.
